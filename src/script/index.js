function toggleFunc(param) {
  if (param == "menu") {
    console.log(
      "%cDEV",
      "background: #3e74ff; border-radius: 4px 0; padding: 4px;color: white",
      `Functionality toggled as [menu]`
    );
  } else if (param == "app") {
    console.log(
      "%cDEV",
      "background: #3e74ff; border-radius: 4px 0; padding: 4px;color: white",
      `Functionality toggled as [app]`
    );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log(
    "%cCRASH",
    "background: #61aaff;  border-radius: 4px 0; padding: 4px;color: white",
    `PC Degredation Report: Crash Rate clocked at ${int}%`
  );
});
...
function disabled(id) {
  ...
  if (id == 1) {
    ...
    let iconTimer = setTimeout(() => {
      document
        .querySelector(".taskbar-icon.taskF")
    ...
    let iconTimer = setTimeout(() => {
      document
        .querySelector(".taskbar-icon.taskT")
        .classList.remove("disabled");
    }, 1000);
  }
}
...
function toggleWindow(id) {
  if (id == 1) {
    ...
    console.log(
      "%cERR",
      "background: #f55;  border-radius: 4px 0; padding: 4px;color: white",
      `No hay una ventana que existe con ese identificacíon.`
    );
  }
}
...
function startApp() {
  console.log(
    "%cPROGRESS",
    "background: #1c9d76;  border-radius: 4px 0; padding: 4px;color: white",
    `DXApp loading. Please wait...`
  );
      ...
      progressCheck = setInterval(() => {
        if (barWidth > 0) {
          document.getElementById(
            "startupText"
          ).innerHTML = `(${barWidth}%) Watching Squeejii play Rocket League...`;
        }
        if (barWidth >= 25) {
          document.getElementById(
            "startupText"
          ).innerHTML = `(${barWidth}%) Playing Kombat League...`;
        }
        if (barWidth >= 50) {
          document.getElementById(
            "startupText"
          ).innerHTML = `(${barWidth}%) Doing AP Chem Homework...`;
        }
        if (barWidth >= 75) {
          document.getElementById(
            "startupText"
          ).innerHTML = `(${barWidth}%) Coding Alluvirum...`;
        }
        if (barWidth >= 95) {
          document.getElementById(
            "startupText"
          ).innerHTML = `(${barWidth}%) Listening to Playboi Carti...`;
        }
      }, 50);
...
function getChromeVersion() {
  ...
  document.querySelector(".chromeVersion").innerHTML = `${
    raw ? parseInt(raw[2], 10) : false
  }.0`;
}
...
console.log(
  "%cNEW VAR",
  "background: #ffe300;  border-radius: 4px 0; padding: 4px;color: white",
  `New Client ID logged as ${clientId}`
);

localStorage.setItem("localId", clientId);

if (clientId != localStorage.getItem("localId")) {
  document.querySelector(".clientId").innerHTML =
...
function copyLink(linkId) {
  if (linkId == 1) {
    ...
    console.log(
      "%cUSER",
      "background: #047eff;  border-radius: 4px 0; padding: 4px;color: white",
      `Twitter link copied.`
    );
  } else if (linkId == 2) {
    navigator.clipboard.writeText(
      `https://www.youtube.com/channel/UCBHkCLZ5cgBb8FGqO9-5rdA`
    );
    console.log(
      "%cUSER",
      "background: #047eff;  border-radius: 4px 0; padding: 4px;color: white",
...
function toggleOpt(opt) {
  if (opt == 1) {
    console.log(
      "%cSTORAGE",
      "background: #ff9b1e;  border-radius: 4px 0; padding: 4px;color: white",
      `Saved as array: [username: "undefined", password: "null"]`
    );
  } else if (opt == 2) {
    console.log(
      "%cSTORAGE",
    ...
    console.log(
      "%cERR",
      "background: #f55;  border-radius: 4px 0; padding: 4px;color: white",
      `No hay un opcion que existe con ese identificacíon.`
    );
  }
}
...
function goIframe() {
  ...
  console.log(
    "%cGET",
    "background: #c683ff;  border-radius: 4px 0; padding: 4px;color: white",
    `Attempting to connect to https://${searchInput} [5]`
  );
}
...
function crash(type) {
  if (type == "normal") {
    console.log(
      "%cFATAL ERROR",
      "background: #f00;  border-radius: 4px 0; padding: 4px;color: white",
      `Website has crashed. Please reload or wait until DXOS is done analysing the problem. Estimated time: Over 3 billion years`
    );
...
function getSongInfo(id) {
  if (id == 1) {
    document.getElementById(
      `musicControls`
    ).style.background = `linear-gradient(to bottom, #00000085, #00000085), url(${songArray.song1.src})`;
    ...
    console.log(
      "%cUSER",
      "background: #047eff;  border-radius: 4px 0; padding: 4px;color: white",
      `Playing Freestyle 2 by Ken Carson.`
    );
  } else if (id == 2) {
    document.getElementById(
      `musicControls`
    ).style.background = `linear-gradient(to bottom, #00000085, #00000085), url(${songArray.song2.src})`;
    ...
    console.log(
      "%cUSER",
      "background: #047eff;  border-radius: 4px 0; padding: 4px;color: white",
      `Playing pick up the phone by Young Thug and Travis Scott.`
    );
  } else {
    console.log(
      "%cERR",
